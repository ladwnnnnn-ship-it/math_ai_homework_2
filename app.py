...
if st.session_state.user is None:
    ...
    if uploaded_file and st.button("开始分析"):
        with st.spinner("AI分析中..."):
            try:
            # 这里根据你买的平台改 base_url 和 model
                client = OpenAI(
                    api_key=st.secrets["THIRD_API_KEY"],      # ← 你在第三方买的 Key
                    base_url=st.secrets["THIRD_BASE_URL"]     # ← 第三方提供的地址import streamlit as st
from supabase import create_client, Client
...
if st.session_state.user is None:
    ...
    if uploaded_file and st.button("开始分析"):
        with st.spinner("AI分析中..."):
            try:
                ...
                # 保存到数据库（用 service_role key 绕过 RLS）
                service_supabase = create_client(supabase_url, st.secrets["SUPABASE_SERVICE_KEY"])
    ...
    if st.button("总结薄弱点"):
        ...
        if records:
            ...
            with st.spinner("汇总中..."):
                # 这里可以用 n1n.ai 也可以继续用 Gemini，建议保持一致
